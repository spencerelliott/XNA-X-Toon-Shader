...
namespace comp4900_xtoon
{
    ...
    {
        Gui gui;
        ToggleButton useToonButton, drawOutlineButton;
        const float maxEdgeWidth = 10.0f;
        const float maxEdgeIntensity = 10.0f;
        Slider edgeWidth, edgeIntensity;
        Label edgeWidthLabel, edgeIntensityLabel;
        ...
        public void Initialize(Game1 game)
        {
            ...
            gui = new Gui(game, skin, text)
            {
                Widgets = new Widget[] {
                    ...
                    edgeWidth = new Slider(margin, margin + (buttonHeight * 2), 150, delegate(Widget slider) {
                        edgeWidthLabel.Value = "Edge Width = " + (((Slider)slider).Value * maxEdgeWidth);
                    }) {
                        Value = 1 / maxEdgeWidth
                    },
                    edgeWidthLabel = new Label(margin, margin + (buttonHeight * 3), "Edge Width = 1.0"),
                    edgeIntensity = new Slider(margin, margin + (buttonHeight * 4), 150, delegate(Widget slider) {
                        edgeIntensityLabel.Value = "Edge Intensity = " + (((Slider)slider).Value * maxEdgeIntensity);
                    }) {
                        Value = 1 / maxEdgeIntensity
                    },
                    edgeIntensityLabel = new Label(margin, margin + (buttonHeight * 5), "Edge Intensity = 1.0")
                }
            };
        }
